...
const navigation = [
  ...
  { name: 'Remote Management', href: '/tenant/remote', icon: Terminal },
  { name: 'Network', href: '/tenant/network', icon: Network },
  { name: 'Reports', href: '/tenant/reports', icon: FileText },
  { name: 'Users', href: '/tenant/users', icon: Users },
];
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-sidebar border-r border-sidebar-border">
        <div className="flex items-center gap-2 h-16 px-6 border-b border-sidebar-border">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-sm text-sidebar-foreground">Fastech</span>
            <span className="text-xs text-sidebar-foreground/60">{profile?.organization_name || 'Tenant'}</span>
          </div>
        </div>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                <SheetContent side="left" className="w-64 p-0">
                  <div className="flex items-center gap-2 h-16 px-6 border-b">
                    <Shield className="w-5 h-5 text-primary" /><span className="font-bold">Fastech</span>
                  </div>
